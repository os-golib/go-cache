...
func ecommerceCacheExample() {
	...
	userCache, _ := cache.NewRedisAdvanced[User]("redis://localhost:6379/0")
	productCache, _ := cache.NewRedisAdvanced[Product]("redis://localhost:6379/0")
	cartCache, _ := cache.NewRedisAdvanced[Cart]("redis://localhost:6379/0")
...
func cacheWithRetry() {
	...
	acache, err := cache.NewRedisAdvanced[string]("redis://localhost:6379/0")
	if err != nil {
		log.Fatal(err)
	}
	defer acache.Close()
...
func batchOperationsExample() {
	...
	err = acache.SetMany(batchCtx, items, 30*time.Minute)
	if err != nil {
	...
	results, err := acache.GetMany(getBatchCtx, keys)
	if err != nil {
...
func hierarchicalKeysExample() {
	...
	acache, _ := cache.NewRedisAdvanced[map[string]interface{}]("redis://localhost:6379/0")
	defer acache.Close()
